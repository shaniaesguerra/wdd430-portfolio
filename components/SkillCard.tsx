    ...
    return (    
        <div className="flex flex-col shadow-md bg-card rounded-xl p-4 mb-4">
            ...
            {technologies.map((tech, index) => (
                    <span key={index} className="bg-white text-card text-sm font-bold font-mono px-3 py-1 rounded-full drop-shadow-[0_0_2px_var(--foreground)]">{tech.tech} - {tech.level}</span>
                ))}
            </section>
        </div>
    );
